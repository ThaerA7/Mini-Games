...
function fitCandidates(pattern: (string | null)[], words: Entry[]) {
  const regex = new RegExp(
    "^" + pattern.map((ch) => (ch ? ch : "[A-Z]")).join("") + "$",
  );
  return words.filter(
    (w) => w.answer.length === pattern.length && regex.test(w.answer),
  );
}
...
function extractPattern(
  ...
  for (let i = 0; i < slot.length; i++) {
    ...
    const cc = slot.c + (slot.dir === "across" ? i : 0);
    seq.push(
      grid[rr][cc] && grid[rr][cc] !== "#" ? (grid[rr][cc] as string) : null,
    );
  }
  return seq;
}
...
function backtrack(
  ...
  const candidates = fitCandidates(pattern, dict).filter(
    (e) => !used.has(e.answer),
  );
...
export function generateCrossword(size = 11): Crossword {
  if (size !== 11) {
    throw new Error(
      "This sample generator currently supports size=11 only (pattern-based).",
    );
  }
  ...
  // Prepare grids
  const solutionGrid: (string | "#")[][] = Array.from(
    { length: size },
    (_, r) => Array.from({ length: size }, (_, c) => (blocks[r][c] ? "#" : "")),
  );
  ...
  const puzzleGrid = Array.from({ length: size }, () =>
    Array.from({ length: size }, () => ""),
  );

  // Convert solution to string[][]
  const solutionGridStrings = filled.map((row) =>
    row.map((ch) => ch as string),
  );
