...
export default function MapShapeBoard({ phase, question, onSubmit }: MapShapeBoardProps) {
  ...
  return (
    ...
    >
      {/* Country outline */}
      <div style={{ display: "grid", placeItems: "center", padding: "18px 8px 10px" }}>
        <div style={{ filter: "drop-shadow(0 4px 12px rgba(0,0,0,.35))", lineHeight: 0 }} aria-label="map-shape">
          {question ? (
            <CountryShape code={question.code} width={320} height={240} />
          ) : (
            <div style={{ width: 320, height: 240, borderRadius: 8, background: "rgba(255,255,255,0.06)" }} />
          )}
        </div>
      </div>
      ...
      >
        ...
        <datalist id={suggestionsId}>
          {guess.trim().length >= 2 &&
            COUNTRIES.filter((c) => c.name.toLowerCase().includes(guess.trim().toLowerCase()))
              .slice(0, 20)
              .map((c) => (
