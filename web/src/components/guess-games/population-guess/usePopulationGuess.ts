...
function makeQuestion(code: string, name: string, flag: string): PopQuestion | null {
  ...
  return { code, flag, country: name, population: pop, acceptedMin, acceptedMax };
}
...
export function usePopulationGuess() {
  ...
  const [pending, setPending] = React.useState<typeof POOL_RAW>(() => shuffle(POOL_RAW));
